...
export default function DashboardScreen() {
  ...
  const todoStats = useMemo(() => {
    ...
    const pct = total > 0 ? Math.round((completed / total) * 100) : 0;
    return { total, completed, active, overdue: overdueCount, highPriority, pct };
  }, [todos]);
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={[styles.content, { paddingTop: topInset + 16, paddingBottom: bottomPad }]}
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerBadges}>
          ...
          {todos.length > 0 && (
            <View style={[styles.headerBadge, { backgroundColor: colors.inProgress + '20' }]}>
              <Text style={[styles.headerBadgeText, { color: colors.inProgress, fontFamily: 'Inter_600SemiBold' }]}>
      ...
      {/* ── Scripts KPI ── */}
      <View style={[styles.kpiCard, { backgroundColor: colors.card, borderRadius: colors.radius, borderColor: colors.border }]}>
        {/* Card header */}
        <View style={styles.kpiCardHead}>
          <Text style={[styles.kpiCardTitle, { color: colors.foreground, fontFamily: 'Inter_700Bold' }]}>
            Scripts
          </Text>
          <Text style={[styles.kpiCardSub, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>
            {stats.pct}% complete
          </Text>
        </View>
        ...
        <View style={styles.kpiBody}>
          ...
          <View style={styles.kpiGrid}>
            ...
            <View style={styles.kpiRow}>
              ...
              <StatCard label="Idle"     value={stats.notStarted} accent={colors.mutedForeground} small />
            </View>
            <View style={styles.kpiRow}>
              <StatCard
                label="Overdue"
                value={stats.overdue}
                accent={stats.overdue > 0 ? colors.destructive : colors.mutedForeground}
                small
              />
              <View style={{ flex: 1 }} />
            </View>
      ...
      <View style={[styles.kpiCard, { backgroundColor: colors.card, borderRadius: colors.radius, borderColor: colors.border }]}>
          ...
          <View
            style={[
              styles.progressFill,
              {
                ...
                backgroundColor: todoStats.pct === 100 ? colors.completed : colors.inProgress,
              },
            ]}
          />
        </View>
        ...
        <View style={styles.todoStatsRow}>
          <StatCard label="Total"    value={todoStats.total}     accent={colors.primary}          small />
          <StatCard label="Done"     value={todoStats.completed} accent={colors.completed}         small />
          <StatCard label="Active"   value={todoStats.active}    accent={colors.inProgress}        small />
          <StatCard
            label="Overdue"
            value={todoStats.overdue}
            accent={todoStats.overdue > 0 ? colors.destructive : colors.mutedForeground}
            small
          />
        </View>
        ...
        {todoStats.highPriority > 0 && (
          <View style={[styles.highPriorityBanner, { backgroundColor: colors.destructive + '15', borderColor: colors.destructive + '40' }]}>
            <Text style={[styles.highPriorityText, { color: colors.destructive, fontFamily: 'Inter_600SemiBold' }]}>
              ⚡ {todoStats.highPriority} high-priority task{todoStats.highPriority > 1 ? 's' : ''} pending
            </Text>
            <Pressable onPress={() => router.push('/(tabs)/todos')}>
              <Text style={[styles.highPriorityLink, { color: colors.destructive, fontFamily: 'Inter_500Medium' }]}>View →</Text>
            </Pressable>
          </View>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  headerBadges: { flexDirection: 'row', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' },
